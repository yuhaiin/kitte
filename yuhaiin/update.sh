...
# openwrt
cat ../geoip/geoip/CN.conf | sed 's/$/ DIRECT,tag=CN/g' > yuhaiin_openwrt.conf
cat ../geoip/geoip/PRIVATE.conf | sed 's/$/ DIRECT,tag=LAN/g' >> yuhaiin_openwrt.conf
...
cat ../geoip/geoip/CN.conf | sed 's/$/ DIRECT,tag=CN/g' >> yuhaiin.conf
cat ../geoip/geoip/PRIVATE.conf  | sed 's/$/ DIRECT,tag=LAN/g' >> yuhaiin.conf
...
# custom
cat ../self/custom.conf | grep -v '^.* BLOCK$' | sed '/^#.*$/'d | sed '/^$/d' >> yuhaiin.conf
cat ../self/custom.conf | sed '/^#.*$/'d >> yuhaiin_ad.conf
cat ../self/block.conf | sed '/^#.*$/'d >> yuhaiin_ad.conf
# cat ../self/custom.conf | sed '/^#.*$/'d >> yuhaiin_openwrt.conf
